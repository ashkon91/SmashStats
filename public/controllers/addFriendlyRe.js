...
.controller('AddFriendlyReCtrl', function($rootScope, $scope, $http, SmashServices, Auth, User) {
...
$scope.openToast = function(){
	...
	if($scope.results.result == 'win'){
		$scope.matchResult = $scope.results.user_char_selected['name'] + " beat " + $scope.results.opp_char_selected['name'] + " on " + $scope.results.stage;
	}
	else{
		$scope.matchResult = $scope.results.opp_char_selected['name'] + " beat " + $scope.results.user_char_selected['name'] + " on " + $scope.results.stage;
	}
}
...
function addMatch(result) {
	...
	$scope.currUser['Stage'][body.stage][body.result]++;
	if(!$scope.currUser.hasOwnProperty('Opponents')){
		$scope.currUser['Opponents'] = {}
	}

	if(!$scope.currUser['Opponents'].hasOwnProperty(body.opp)){
		$scope.currUser['Opponents'][body.opp] = {"Matches": []}
	}
	jsonObj = {
		"uChar": body.uChar,
		"oChar": body.oChar,
	...
	$scope.currUser['Opponents'][body.opp]['Matches'].push(jsonObj)
	$scope.currUser.$save();
}
